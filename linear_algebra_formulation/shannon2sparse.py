...
def collect_symbolic_shannon_model(
    ...
    z_probability_rows = {key: sparse_row_from_dict(row, d) for key, row in z_probability_sparse.items()}
    h_probability_rows = {key: sparse_row_from_dict(row, d) for key, row in h_probability_sparse.items()}
    h_rho_rows = {key: sparse_row_from_dict(row, d) for key, row in h_rho_sparse.items()}
    ...
    z_rho_rows = {
        key: sparse_row_from_dict(row, d)
